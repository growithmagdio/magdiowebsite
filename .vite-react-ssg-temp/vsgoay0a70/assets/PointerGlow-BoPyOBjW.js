...
//#region src/components/PointerGlow.jsx
globalThis.__VITE_REACT_SSG_TRACK_SSR_MODULE__?.("src/components/PointerGlow.jsx");
function PointerGlow() {
	...
	useEffect(() => {
		...
		const handleMouseMove = (e) => {
			...
			if (speedSq > .25 && mouse.isActive) {
				const count = Math.min(3, Math.floor(Math.sqrt(speedSq) * .08) + 1);
				for (let i = 0; i < count; i++) {
					const isStar = Math.random() < .4;
					const angle = Math.random() * Math.PI * 2;
					const spreadSpeed = Math.random() * 1.2;
					const vx = Math.cos(angle) * spreadSpeed + dx * .1;
					const vy = Math.sin(angle) * spreadSpeed + dy * .1 - .2;
					particles.push({
						x: mouse.x + (Math.random() - .5) * 6,
						y: mouse.y + (Math.random() - .5) * 6,
						vx,
						vy,
						size: isStar ? Math.random() * 3.5 + 2 : Math.random() * 1.2 + .5,
						isStar,
						life: 1,
						decay: isStar ? Math.random() * .015 + .01 : Math.random() * .03 + .02,
		...
		const animate = () => {
			...
			if (mouse.isActive) {
				...
				const ambientGrad = ctx.createRadialGradient(mouse.x, mouse.y, 0, mouse.x, mouse.y, spotRadius);
				const alphaBase = isHoveredRef.current ? .22 : .14;
				ambientGrad.addColorStop(0, `rgba(26, 34, 184, ${alphaBase})`);
				ambientGrad.addColorStop(.35, `rgba(138, 43, 226, ${alphaBase * .65})`);
				ambientGrad.addColorStop(.65, `rgba(242, 179, 0, ${alphaBase * .35})`);
			...
			if (mouse.isActive && isHoveredRef.current && Math.random() < .15) particles.push({
				x: mouse.x + (Math.random() - .5) * 20,
				y: mouse.y + (Math.random() - .5) * 20,
				vx: (Math.random() - .5) * .6,
				vy: (Math.random() - .5) * .6 - .3,
				size: Math.random() * 3 + 1.5,
				isStar: Math.random() < .5,
				life: 1,
			...
			for (let i = particles.length - 1; i >= 0; i--) {
				...
				if (p.isStar) {
					const pulse = Math.sin(p.life * Math.PI * 6 + p.twinkleOffset) * .28 + .72;
